<template>
  <div>
    <el-breadcrumb style='padding:24px;padding-left:0' separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/distributorCenter/saleList' }">销售订单</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-table :data="tableData" @sort-change="mysort" border style="width: 100%" slot="empty">
      ...
      <el-table-column prop="takeTeaCount" show-overflow-tooltip align="center" label="操作" width="100">
        <template slot-scope="scope">
          <a @click="$router.openPage('/distributorCenter/billDetail')" style="color:#0166bb" v-if="scope.row.takeTeaCount">已补发票</a>
          <br v-if="scope.row.takeTeaCount">
          <a @click="$router.openPage('/distributorCenter/makeBill')" style="color:#0166bb" v-if="scope.row.orderStatus==2">补开发票</a>
          <br v-if="scope.row.orderStatus==2">
